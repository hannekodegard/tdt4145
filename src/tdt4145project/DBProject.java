...
public class DBProject {
	...
	private void addFilmToDb(Connection con, String tittel, int lengde, int utgivelses�r, String lanseringsdato, String storyline, String Utgivelsesselskap_URL, int serie) {
		//Format utgivelses�r: YYYY-MM-DD

		Statement myStat;
		try {
			myStat = con.createStatement();
			String formatted = String.format(" values ('%s',%s,%s,'%s','%s','%s', %s)", tittel, lengde, utgivelses�r, lanseringsdato, storyline, Utgivelsesselskap_URL, serie);
			String sql = "insert into filmatisering " + " (tittel, lengde, utgivelses�r, lanseringsdato, storyline, Utgivelsesselskap_URL, serie)" + formatted;
			myStat.executeUpdate(sql);
			System.out.println("Insert complete");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	private void addMusicToDb(Connection con, String  komponent, String  fremf�rtAv) {
		Statement myStat;
		try {
			myStat = con.createStatement();
			String formatted = String.format(" values ('%s','%s')", komponent, fremf�rtAv);
			String sql = "insert into Musikk " + " (komponent, fremf�rtAv)" + formatted;
			myStat.executeUpdate(sql);
			System.out.println("Insert complete");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	private void addInvolvedToDb(Connection con,String  navn, int  f�dsels�r ,String  f�dselsland) {
		Statement myStat;
		try {
			myStat = con.createStatement();
			String formatted = String.format(" values ('%s', %s, '%s')", navn, f�dsels�r, f�dselsland);
			String sql = "insert into involvertIFilm " + " (navn, f�dsels�r, f�dselsland)" + formatted;
			myStat.executeUpdate(sql);
			System.out.println("Insert complete");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	private void addActorToDb(Connection con, int involvertIFilm_personNr, int filmatisering_filmID, String rolle) {
		...
		try {
			myStat = con.createStatement();
			String formatted = String.format(" values (%s,%s,'%s')", involvertIFilm_personNr, filmatisering_filmID,
					rolle);
			String sql = "insert into skuespillerIFilm " + " (involvertIFilm_personNr, filmatisering_filmID, rolle)"
					+ formatted;
			myStat.executeUpdate(sql);
			System.out.println("Insert complete");
		} catch (SQLException e) {
	...
	private void addWriterToDb(Connection con, int involvertIFilm_personNr, int filmatisering_filmID) {
		...
		try {
			...
			String sql = "insert into manusforfatterHarSkrevet " + " (involvertIFilm_personNr, filmatisering_filmID)"
					+ formatted;
			myStat.executeUpdate(sql);
			System.out.println("Insert complete");
		} catch (SQLException e) {
	...
	private void addProducerToDb(Connection con, int involvertIFilm_personNr, int filmatisering_filmID) {
		...
		try {
			myStat = con.createStatement();
			String formatted = String.format(" values (%s,%s)", involvertIFilm_personNr, filmatisering_filmID);
			String sql = "insert into regiss�rRegissertFilm " + " (involvertIFilm_personNr, filmatisering_filmID)"
					+ formatted;
			myStat.executeUpdate(sql);
			System.out.println("Insert complete");
		} catch (SQLException e) {
	...
	private void updateMovie(Connection con, String movieName, String URL) {
		...
		System.out.println("Hvilket �r ble den utgitt?");
		int utgivelses�r = Integer.parseInt(myObj.nextLine());  // Read user input
		...
		if (erserie.equals("Ja") || erserie.equals("ja")) {
			...
			if (erEpisode.equals("Ja") || erEpisode.equals("ja")) {
				System.out.println("Hvilken serie tilh�rer episoden?");
				String filmNavn = myObj.nextLine();  // Read user input
				System.out.println("Hvilken sesong tilh�rer den?");
		...
		this.addFilmToDb(con, movieName, lengde, utgivelses�r, lanseringsdato, storyline, URL, serie);
	...
	private void addActors(Connection con, int filmID) {
		...
		for(int i = 0; i <= antallSkuespillere; i++) {
			...
			if (checkActor == -1) {
				System.out.println("Hva er f�dsels�ret til skuespilleren?");
				int f�dsels�r = Integer.parseInt(myObj.nextLine());  // Read user input
				System.out.println("Hvilket land er skuespilleren fra?");
				String land = myObj.nextLine();  // Read user input
				this.addInvolvedToDb(con, skuespillerNavn, f�dsels�r, land);
	...
	private void addWriter(Connection con, int filmID) {
		...
		for(int i = 1; i <= antallSkribenter; i++) {
			...
			if (checkActor == -1) {
				System.out.println("Hva er f�dsels�ret til skribenten?");
				int f�dsels�r = Integer.parseInt(myObj.nextLine());  // Read user input
				System.out.println("Hvilket land er skribenten fra?");
				String land = myObj.nextLine();  // Read user input
				this.addInvolvedToDb(con, skribentNavn, f�dsels�r, land);
	...
	private void addProducer(Connection con, int filmID) {
		...
		if (checkActor == -1) {
			System.out.println("Hva er f�dsels�ret til produsenten?");
			int f�dsels�r = Integer.parseInt(myObj.nextLine());  // Read user input
			System.out.println("Hvilket land er produsenten fra?");
			String land = myObj.nextLine();  // Read user input
			this.addInvolvedToDb(con, produsentNavn, f�dsels�r, land);
			}
	...
	private void addMusic(Connection con, int filmID) {
		...
		if (antallSkribenter > 0) {
			for(int i = 1; i <= antallSkribenter; i++) {
				...
				System.out.println("Hvem er sangen fremf�rt av?");
				String fremf�rtAv = myObj.nextLine();  // Read user input
				this.addMusicToDb(con, komponist, fremf�rtAv);
	...
	private void addFormat(Connection con, int filmID) {
		...
		System.out.println("Kan filmen vises p� kino?");
		String stream = myObj.nextLine();
		if (stream.toUpperCase().equals("JA")) {
			addFilmHasFormatToDb(con, filmID, "Kino");
		}
		System.out.println("Kan filmen ses p� TV?");
		String stream = myObj.nextLine();
		if (stream.toUpperCase().equals("JA")) {
	...
	private void showDbInfo(Connection con) {
		...
		try {
			...
			while (statement.next()) {
				System.out.println(statement.getString("URL") + ", " + statement.getString("Adresse") + ", "
						+ statement.getString("Land"));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		// dbproject.addFilmToDb(con,"Mamma Mia", 200, 2018, "2018-12-20", "Music",
		// "DreamWorks");
		// dbproject.showDbInfo(con);
		// dbproject.addInvolvedToDb(con, 2345, "Niklas", 1996, "Norge");
		// dbproject.addCategoryToDb(con, 1, "Gr�sser");
		// dbproject.addActorToDb(con, 23456, 1, "Teddybj�rn");
